...
<script type="text/javascript">
	$('.keranjang-button').on('click', function(){
		...
		if(!empty($result->id_user)){ ?>
			$.ajax({
				url: "<?= base_url() ?>user/keranjang/store/"+$(this).data('id')+"/"+$(this).val(),
				type: "POST",
					...
					$.ajax({
						...
						success: function (data) {
							...
							for (var i = 0; i < datas.length; i++) {
								...
								'<td class="text-center"><button onclick="deleteFunction('+datas[i].id_keranjang+')" data-toggle="tooltip" id="hapus_keranjangs" value="'+datas[i].id_keranjang+'"'+ 
								'title="" class="btn btn-danger btn-xs hh" data-original-title="Hapus"><i class="fa fa-trash-o"></i></button></td>'+
								'<tr>';
								$('#tabel-keranjang').append(row);
							}
						},
